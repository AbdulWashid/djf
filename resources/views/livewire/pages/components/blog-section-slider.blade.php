...
new class extends Component {
    ...
    public function mount($limit = 5, $featuredOnly = false, $categorySlug = null)
    {
        ...
        $this->categories = Category::active()->withCount('posts')->having('posts_count', '>', 0)->orderBy('name')->get();
    ...
    public function loadArticles()
    {
        ...
        $this->articles = $query->orderBy('published_at', 'desc')->limit($this->limit)->get();
    }
                ...
                <div class="container-default">
                    ...
                    <div class="mb-12 text-center">
                        ...
                        <p class="max-w-3xl mx-auto text-primary-700">
                            Discover tips, tutorials, and insights about building powerful applications with Laravel and
                            Filament.
                        </p>
                    </div>
                    ...
                    <div class="relative" wire:loading.class="opacity-50 pointer-events-none">
                        ...
                        <div wire:loading class="absolute inset-0 z-50 flex items-center justify-center bg-opacity-60">
                            <svg class="w-10 h-10 text-primary-600 animate-spin" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <button id="article-slider-prev"
                            class="absolute left-0 z-20 flex items-center justify-center w-12 h-12 text-white transition-all duration-300 -translate-x-6 -translate-y-1/2 shadow-lg cursor-pointer bg-primary-700 articles-slider-prev top-1/2 hover:scale-110 hover:bg-primary-800 focus:outline-none focus:ring-2 focus:ring-primary-400 focus:ring-offset-2">
                        ...
                        <button id="article-slider-next"
                            class="absolute right-0 z-20 flex items-center justify-center w-12 h-12 text-white transition-all duration-300 translate-x-6 -translate-y-1/2 shadow-lg cursor-pointer bg-primary-700 articles-slider-next top-1/2 hover:scale-110 hover:bg-primary-800 focus:outline-none focus:ring-2 focus:ring-primary-400 focus:ring-offset-2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M9 5l7 7-7 7" />
                            ...
                            <div class="swiper-wrapper">
                                @forelse($articles as $article)
                                    ...
                                    <div class="swiper-slide">
                                        <div
                                            class="flex flex-col h-full overflow-hidden transition-all duration-300 bg-white border shadow-lg border-background-light group hover:shadow-xl">
                                            <a href="{{ route('blog.show', ['slug' => $article->slug]) }}"
                                                ...
                                                <div class="relative overflow-hidden aspect-video">
                                                    ...
                                                    @endif
                                                    <div
                                                        class="absolute inset-0 transition-opacity bg-gradient-to-t from-primary-900/60 via-primary-900/20 to-transparent opacity-60 group-hover:opacity-70">
                                                    </div>
                                                    <div class="absolute bottom-0 left-0 right-0 p-5">
                                                        <span
                                                            class="inline-block px-3 py-1 text-xs font-semibold tracking-wider text-white uppercase shadow-md"
                                                            style="background-color: {{ $article->category->options['color'] ?? '#2D2B8D' }}">
                                                            {{ $article->category->name }}
                                                        </span>
                                                        @if ($article->is_featured)
                                                            <span
                                                                class="inline-block px-3 py-1 ml-2 text-xs font-semibold tracking-wider text-white uppercase shadow-md bg-secondary-600">
                                                                Featured
                                                            </span>
                                                        @endif
                                                    </div>
                                            ...
                                            <div class="flex flex-col flex-grow p-6">
                                                ...
                                                <h3
                                                    class="mb-3 text-xl font-bold transition-colors text-primary-800 group-hover:text-primary-600">
                                                    <a href="{{ route('blog.show', ['slug' => $article->slug]) }}"
                                                        wire:click="trackView('{{ $article->id }}')">
                                                        {{ $article->title }}
                                                    </a>
                                                </h3>
                                                <p class="flex-grow mb-5 text-neutral-600">
                                                    {{ $article->content_overview }}</p>
                                                <div
                                                    ...
                                                    <div class="flex items-center">
                                                        ...
                                                        @endif
                                                        <span
                                                            class="text-sm font-medium text-primary-700">{{ $article->author->name ?? 'Anonymous' }}</span>
                                                    </div>
                                                    <a href="{{ route('blog.show', ['slug' => $article->slug]) }}"
                                                        wire:click="trackView('{{ $article->id }}')"
                                                        class="flex items-center text-sm font-medium transition-colors text-primary-600 hover:text-primary-700">
                                                        Read More
                                    ...
                                    <div class="w-full p-10 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                class="w-16 h-16 mb-4 text-neutral-300" fill="none"
                    ...
                    <div class="mt-12 text-center">
                        <a href="{{ route('blog') }}"
                            class="inline-block px-6 py-3 font-medium transition-all duration-300 border-2 text-primary-700 border-primary-600 hover:bg-primary-600 hover:text-white group">
                            <span class="flex items-center">
                                View All Articles
                                <svg xmlns="http://www.w3.org/2000/svg"
    ...
    @push('css')
        <style>
            .articles-slider-pagination {
                --swiper-pagination-color: #2D2B8D;
                /* Primary-800 */
                --swiper-pagination-bullet-size: 10px;
                --swiper-pagination-bullet-inactive-color: #DCDCF7;
                /* Primary-100 */
