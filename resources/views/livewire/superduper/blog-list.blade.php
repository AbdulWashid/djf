<div>
    ...
    <div wire:loading.delay
        ...
        <svg class="w-5 h-5 text-primary-600 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none"
            viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 gap-x-6 gap-y-10 lg:grid-cols-[1fr,minmax(416px,_0.45fr)]">
                                ...
                                <div class="flex overflow-hidden border rounded">
                                    <button wire:click="sortBy('published_at')"
                                        ...
                                        @if ($sortField === 'published_at')
                                            <i
                                                class="fa-solid fa-chevron-{{ $sortDirection === 'desc' ? 'down' : 'up' }} ml-1 text-xs"></i>
                                        @endif
                                    </button>
                                ...
                                <div class="jos" wire:key="post-{{ $post->id }}">
                                    <div
                                        ...
                                        <div class="p-[30px]">
                                            ...
                                            <ul
                                                ...
                                                <li>
                                                    <a href="#"
                                                        ...
                                                        class="rounded-[50px] bg-color-black/5 px-[26px] py-1.5 text-color-black/60 hover:bg-color-blue hover:text-white">
                                                        {{ $post->category->name }}
                                                    </a>
                                                </li>
                                                @if ($post->is_featured)
                                                    <li>
                                                        <span
                                                            class="rounded-[50px] bg-orange-500 px-[26px] py-1.5 text-white">
                                                            Featured
                                                        </span>
                                                    </li>
                                                @endif
                                            ...
                                            <a href="{{ $post->getUrl() }}"
                                                wire:click="trackView('{{ $post->id }}')"
                                                class="inline-flex items-center text-base font-bold gap-x-2 text-color-black group-hover:text-primary-600">
                                                Read More
                                                <span
                                ...
                                <ul class="flex gap-x-[15px]">
                                    ...
                                    <li>
                                        <button wire:click="previousPage"
                                            ...
                                            class="group flex h-10 w-10 items-center justify-center rounded-[50%] {{ $posts->onFirstPage() ? 'bg-gray-200 cursor-not-allowed' : 'bg-white hover:bg-color-blue hover:text-white' }} font-semibold transition-all duration-300 lg:h-[50px] lg:w-[50px]">
                                    ...
                                    @foreach ($posts->getUrlRange(1, $posts->lastPage()) as $page => $url)
                                        <li>
                                            <button wire:click="gotoPage({{ $page }})"
                                                class="group flex h-10 w-10 items-center justify-center rounded-[50%] {{ $page == $posts->currentPage() ? 'bg-color-blue text-white' : 'bg-white hover:bg-color-blue hover:text-white' }} font-semibold transition-all duration-300 lg:h-[50px] lg:w-[50px]">
                                                {{ $page }}
                                            </button>
                                        </li>
                                    @endforeach
                                    ...
                                    <li>
                                        <button wire:click="nextPage"
                                            ...
                                            class="group flex h-10 w-10 items-center justify-center rounded-[50%] {{ !$posts->hasMorePages() ? 'bg-gray-200 cursor-not-allowed' : 'bg-white hover:bg-color-blue hover:text-white' }} font-semibold transition-all duration-300 lg:h-[50px] lg:w-[50px]">
                    ...
                    <aside class="lg:sticky lg:top-8 lg:self-start flex flex-col gap-y-[30px]">
                        ...
                        <div class="rounded-[10px] bg-color-off-white p-5">
                            ...
                            <form class="relative h-[60px]" wire:submit.prevent="$refresh" role="search"
                                ...
                                <div class="absolute left-[30px] top-0 h-full flex items-center text-gray-500"
                                    aria-hidden="true">
                        ...
                        <div class="rounded-[10px] bg-color-off-white p-8">
                            <div
                                class="relative mb-[30px] inline-block pb-[10px] text-lg font-semibold text-color-black after:absolute after:bottom-0 after:left-0 after:h-[2px] after:w-full after:bg-black">
                                Recent Posts
                            </div>
                                ...
                                @foreach ($recentPosts as $recentPost)
                                    <li class="flex flex-col items-center group gap-x-4 gap-y-4 sm:flex-row">
                                        ...
                                        <div class="flex flex-col w-full gap-y-3 sm:w-auto sm:flex-1">
                                            <a href="{{ $recentPost->getUrl() }}"
                                                class="flex items-center gap-[10px] text-sm hover:text-primary-600"
                                                wire:click="trackView('{{ $recentPost->id }}')">
                                                <i class="fa-regular fa-calendar"></i>
                                                {{ $recentPost->published_at->format('M d, Y') }}
                                            </a>
                                            <a href="{{ $recentPost->getUrl() }}"
                                                class="text-base font-semibold line-clamp-2 text-color-black hover:text-primary-600"
                                                wire:click="trackView('{{ $recentPost->id }}')">
                                                {{ $recentPost->title }}
                                            </a>
                                        </div>
                        ...
                        <div class="rounded-[10px] bg-color-off-white p-8 relative">
                            ...
                            <div wire:loading.delay wire:target="filterByCategory"
                                class="absolute text-primary-600 right-8 top-8">
                            ...
                            <ul class="text-color-black">
                                <li
                                    class="border-b border-color-black/10 pb-[14px] pt-[14px] first:pt-0 last:border-b-0 last:pb-0">
                                    <button wire:click="filterByCategory(null)"
                                        class="w-full text-left {{ is_null($activeCategory) ? 'text-primary-600 font-semibold' : 'hover:text-primary-600' }}">
                                        All Categories
                                    </button>
                                </li>
                                @foreach ($categories as $category)
                                    <li
                                        ...
                                        <button wire:click="filterByCategory('{{ $category->id }}')"
                                            class="w-full text-left {{ $activeCategory === $category->id ? 'text-primary-600 font-semibold' : 'hover:text-primary-600' }}">
                                ...
                                <li
                                    ...
                                    <button wire:click="toggleFeatured"
                                        ...
                                        Featured Only
                                        <span wire:loading.delay wire:target="toggleFeatured"
                                ...
                                @forelse($popularTags as $tag)
                                    <li>
                                        <button
                                            ...
                                            <span
                                                class="ml-1.5 flex items-center justify-center text-xs h-5 w-5 rounded-full bg-gray-200 text-gray-700">
