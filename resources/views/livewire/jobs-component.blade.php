<div>
    ...
    <section class="section-box-2">
        <div class="box-head-single none-bg">
            <div class="container">
                <h4>Currently available Jobs @if(!empty($location)) in {{Str::ucwords($location)}}@endif!</h4>
            </div>
        </div>
    </section>
                                ...
                                <div class="col-lg-4 col-md-6">
                                    <div class="card-grid-2 hover-up wow animate__animated animate__fadeIn"
                                        ...
                                        <div class="card-block-info">
                                                    ...
                                                    <a href="{{ route('jobs.show', $job->slug) }}"
                                                       class="card-2-img-text card-grid-2-img-medium">
                                                        <span class="card-grid-2-img-small"><img
                                                                src="{{ $job->employer->logo ? Storage::url($job->employer->logo) : Storage::url($generalSettings->site_favicon)  }}"/></span>
                                                        <span>{{ $job->title }}</span>
                                                    </a>
                                            ...
                                            <div class="mt-15">
                                                <span class="text-sm text-mutted-2 mr-20"><i class="fi-rr-marker"></i> {{ $job->location }}</span>
                                                <span class="text-sm text-mutted-2"><i class="fi-rr-briefcase"></i> {{ $job->job_type->getLabel() }}</span>
                                            </div>
                                            ...
                                            <div class="card-2-bottom mt-30">
                                                <div class="row">
                                                    ...
                                                    <div class="col-lg-6 col-4 text-end">
                                                        <span class="card-text-price"> {{ $job->salary_range }}<span>/year</span> </span>
                                                    </div>
                                                </div>
                                ...
                                <div class="col-12">
                                    <div class="card-grid-2 hover-up wow animate__animated animate__fadeIn">
                                        <div class="card-block-info text-center py-5">
                                            ...
                                            <h5>No jobs found</h5>
                                            <p class="text-muted">We couldn't find any jobs matching your criteria. Try adjusting your filters or clearing them.</p>
                                            <div class="mt-4">
                                                <button wire:click="clear" class="btn btn-default">Clear all filters</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="sidebar-filters">
                            <div class="filter-block mb-30">
                                ...
                                <div class="form-group select-style select-style-icon" wire:ignore>
                                    <select id="location-select" wire:model.live="location" class="location-select form-control form-icons">
                                        <option value="">Select Location</option>
                                        @foreach($locations as $loc)

                                            <option value="{{ $loc}}">{{Str::ucwords($loc)}}</option>
                                        ...
                                        @foreach($job_types as $enum_key => $jobType)
                                            ...
                                            <li>
                                                <label class="cb-container">
                                                    <input type="checkbox" wire:model="job_type" value="{{$enum_key}}"> <span
...
{{--                            </div>--}}
                            <div class="buttons-filter">
                                <button class="btn btn-default" wire:click="search()" type="button">Apply filter</button>
