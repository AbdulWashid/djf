...
new class extends Component {
    ...
    {
        $this->posts = rememberIfEnabled('recent_posts_home', now()->addMinutes(30), function () {
            return Post::published()
                //                ->where('id', '!=', $this->post->id)
                ->select(['id', 'title', 'slug', 'blog_category_id', 'published_at', 'content_overview', 'blog_author_id'])
                ->with(['category:id,name,slug', 'media', 'author:id,username,firstname'])
                                    ...
                                    <div class="card-grid-3 hover-up h-100 d-flex flex-column">
                                        <div class="card-image mb-3 overflow-hidden">
                                            <a href="{{ $recentPost->getUrl() }}">
                                                ...
                                                @else
                                                    <img loading="lazy"
                                                        src="https://placehold.co/1600x900?text={{ urlencode(substr($recentPost->title, 0, 20)) }}"
                                                        alt="{{ $recentPost->title }}"
                                                        class="img-fluid w-100 blog-card-image">
                                                @endif
                                            </a>
