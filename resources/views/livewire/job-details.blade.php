                        ...
                        <div class="row align-items-center">
                            <div class="col-md-5">
                                <a href="#"
                                   ...
                                   wire:click.prevent="openApplyModal"
                                >Apply now</a>
                            </div>
                            <div class="col-md-7 text-lg-end social-share">
                                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->full()) }}" class="btn btn-border btn-sm mr-10"><img alt="{{ $job->title }}" src="{{ asset('assets/imgs/theme/icons/share-fb.svg') }}" /> Share</a>
                                <a href="https://twitter.com/intent/tweet?text=Check out this job {{ trim($job->title) }}&url={{ urlencode(url()->full()) }}" target="_blank" rel="noopener noreferrer" class="btn btn-border btn-sm mr-10"><img alt="{{ $job->title }}" src="{{ asset('assets/imgs/theme/icons/share-tw.svg') }}" /> Tweet</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sidebar-shadow">
                        <div class="sidebar-heading">
                            <div class="avatar-sidebar">
                                <figure><img alt="{{ $job->title }}" src="{{ $job->employer->logo ? Storage::url($job->employer->logo) : Storage::url($generalSettings->site_favicon)  }}" /></figure>
...
{{--                            <a href="#" class="btn btn-border">Save job</a>--}}
                        ...
                        <div class="sidebar-team-member pt-40">
                            ...
                            <div class="info-address">
                                    <span><i class="fi-rr-marker"></i> <span>{{$job->employer->address}}, {{ $job->employer->city }}, {{$job->employer->state}}, {{ $job->employer->country }}</span></span>
                                <span><i class="fi-rr-headset"></i> <span>{{ $job->employer->phone }} </span></span>
                                <span><i class="fi-rr-paper-plane"></i> <span>{{ $job->employer->email }}</span></span>
                            </div>
                        </div>
                        @endif
                    </div>
                ...
                <div class="col-md-12">
                    ...
                    <livewire:faqs-list section="jobs" />
    ...
    @if($showApplyModal ?? false)
        <div
            ...
            style="background: rgba(0,0,0,.55); z-index: 1050;"
        >
            <div
                class="bg-white p-4"
                style="max-width: 720px; margin: 5vh auto; border-radius: 12px;"
                wire:click.stop
            >
                ...
                <form wire:submit.prevent="submitApplication">
                    <div class="row">
                        ...
                        <div class="col-12 mb-3">
                            ...
                            @error('cover_letter') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                    </div>

                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-default" wire:loading.attr="disabled" wire:target="submitApplication,cv">
                            Submit Application
                        </button>
